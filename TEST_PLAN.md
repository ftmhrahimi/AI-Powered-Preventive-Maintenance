...
A clear, end‑to‑end checklist to validate the platform before delivery.
...
- Tests are split by **who can run them**:
  - **Part A — Platform‑only tests**: anything you can do from a **web browser**
    with a login. No server access needed. (Testers, the requester, admins, end
    users.)
  - **Part B — Server‑access tests**: things that need **SSH + Docker + the
    database** on the server. (Operator / deployer.)
- **🔁 Regression** marks a test that guards a previously‑fixed bug. A
  *regression* = something that used to work breaking again after a change. These
  must keep passing on every release (see the Regression Quick‑Suite at the end).

## Glossary
- **Regression** — a previously‑working behaviour that breaks again later.
...
- 5–10 valid report PDFs (mix of sizes; ≥ 3 for the concurrency tests).
...
| ID | Steps | Expected |
|----|-------|----------|
| A15‑1 🔁 | Process a Persian report (including longer items) and read the descriptions | Item text reads correctly — **no garbled/duplicated letters** (e.g. not "کنکد"/"سکسیستم") |
...
| B1‑3 | Confirm latest frontend is served: `sudo docker compose exec frontend grep -c "__saveScope" /usr/share/nginx/html/index.html` | ≥ 1 |
| B1‑4 | Confirm latest backend: `sudo docker compose exec backend grep -c "get_active_server_runs" /app/db.py` | ≥ 1 |
| B1‑5 | Cold start: `sudo docker compose up -d --build` | Whole stack comes up; app reachable |
...
| B5‑7 | Check CORS: a request from a disallowed origin | Blocked per `ALLOWED_ORIGIN` |
| B5‑8 | Inspect the DB `users` table | Passwords are **hashed**, never plaintext |
...
# Regression Quick‑Suite (run before every release)

The minimum set that re‑checks every previously‑fixed bug. All must pass.

1. **A3‑4 / A3‑5** — Run All disabled during upload, re‑enables after.
2. **A3‑6** — Stop → Run causes no re‑upload.
3. **A5‑1 / A5‑2** — Run All starts all at once; several progress in parallel.
4. **A5‑3** — No progress bar reset/jump (each bar is its own file).
5. **A6‑1 / A6‑2 / A6‑3** — Stopping one file never corrupts the others.
6. **A6‑5** — A finished file stays Done through Stop All.
7. **A7‑1** — Close the tab; the run continues server‑side.
8. **A10‑4** — A task rule actually reaches the AI on re‑run.
9. **A12‑2 / A12‑3** — Admin password reset works; user logs in with the new one.
10. **A15‑1** — Persian descriptions render correctly (no garbled letters).
